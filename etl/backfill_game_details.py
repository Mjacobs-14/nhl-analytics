...
Game Details ETL
================
Fills in three things the boxscore pipeline doesn't carry, for every
...
Skip-existing on rerun: games whose attendance AND referees are already
set are skipped unless --force. Shifts are written for whichever players
already have a stat row for that game (never creates phantom rows).

SETUP: run sql/010_shot_events_and_game_details.sql in Supabase first.
Same SUPABASE_URL / SUPABASE_KEY env vars as the other scripts.
...
def get_games_to_process(sb, season=None, limit=None, force=False):
    """Games rows (id, date, season) still missing details, oldest first."""
    games = []
    page_size = 1000
    offset = 0
    while True:
        query = sb.table("games").select("game_id, game_date, season, attendance, referees")
        if season:
    ...
    if not force:
        before = len(games)
        games = [g for g in games if g["attendance"] is None or g["referees"] is None]
...
def fetch_game_details(game):
    """Fetches shifts + officials + attendance for one game. Returns
    (game, shifts_by_player, games_patch, error)."""
...
def main():
    ...
    games = get_games_to_process(sb, season=args.season, limit=args.limit, force=args.force)
    ...
    for start in range(0, len(games), BATCH):
        ...
        known_pairs = get_stat_row_players(sb, [g["game_id"] for g in batch])

        shift_rows = []
        game_patches = []
        with ThreadPoolExecutor(max_workers=args.workers) as executor:
            futures = [executor.submit(fetch_game_details, g) for g in batch]
